...
export default function DeliveryApp() {
  ...
  return (
    <div className={`min-h-screen ${darkMode ? 'bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900' : 'bg-gradient-to-br from-orange-50 via-white to-purple-50'}`}>
        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center gap-2">
              <span className={`text-sm font-medium ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>CONTEXT:</span>
              <select
                value={userMode}
                onChange={(e) => {
              ...
              >
                <option value={UserMode.SENDER}>Request Shipping/Taxi</option>
                <option value={UserMode.COURIER}>Provide Transport</option>
              </select>
            </div>
      ...
      {showSettings && (
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div ref={settingsRef} className={`${darkMode ? 'bg-gray-800 text-white' : 'bg-white'} rounded-xl shadow-lg p-6`}>
            <h2 className={`text-xl font-bold mb-6 ${darkMode ? 'text-white' : 'text-gray-900'}`}>Settings</h2>

            {/* Dark Mode and NWC Wallet - Side by Side */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              ...
              <div className={`p-4 ${darkMode ? 'bg-gray-700' : 'bg-gray-50'} rounded-lg`}>
                <div className="flex flex-col h-full">
                  ...
                  <p className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>Switch theme</p>
                </div>
              </div>

              {/* NWC Wallet Section */}
              <div className={`p-4 ${darkMode ? 'bg-gray-700' : 'bg-gray-50'} rounded-lg`}>
                <h3 className={`font-semibold ${darkMode ? 'text-white' : 'text-gray-900'} mb-2 flex items-center gap-2`}>
              ...
              deliveryRequests.filter(r => r.status === 'open').map(request => (
                <div key={request.id} className={`${darkMode ? 'bg-gray-800 text-white' : 'bg-white'} rounded-xl shadow-lg p-6`}>
                  ...
                  {request.packages && request.packages.length > 0 && (
                    <div className="mb-4">
                      {request.packages.map((pkg, idx) => (
                        <div key={idx} className={`mb-3 p-3 ${darkMode ? 'bg-gray-700' : 'bg-gray-50'} rounded-lg`}>
                          <p className={`text-sm mb-2 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                            <strong>Package Size:</strong> {pkg.size}
                          </p>
                          {pkg.description && (
                            <p className={`text-sm mb-2 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                              <strong>Package Description:</strong> {pkg.description}
                            </p>
                          )}
                  ...
                  {(request as any).persons && ((request as any).persons.adults > 0 || (request as any).persons.children > 0) && (
                    <div className={`mb-4 p-3 ${darkMode ? 'bg-gray-700' : 'bg-gray-50'} rounded-lg`}>
                      <p className={`text-sm mb-2 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                        <strong>Number of Adults:</strong> {(request as any).persons.adults}, <strong>Number of Children:</strong> {(request as any).persons.children}
                      </p>
                      <div className="flex flex-wrap gap-2">
